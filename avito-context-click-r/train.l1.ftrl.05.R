...
tmp <- foreach(tmp=1, .noexport=all.noexport) %dopar% {
  ...
  fn.init.worker("ftrl_05/build_csv")
  ...
  for (jx in 1) { # :2
    ...
    for (test.type in c("val", "test")) { # "tr", 
...
data.ftrl.05.pred.tmp <- foreach(
  ...
  system(paste(
    ...
    "-bits 27 -alpha .008 -beta .1 -l1 0.1 -l2 0.15 -dropout 0",
...
data.ftrl.05.pred.tmp <- data.ftrl.05.pred.tmp[order(ID)]
Store(data.ftrl.05.pred.tmp)

data.ftrl.05.pred <- data.ftrl.05.pred.tmp[
  ,list(
    Pred=sum(Pred)/.N
    ), by="ID"
]

fn.print.err(data.ftrl.05.pred)
#      Size     Loss
# 1 7888752  0.04122 - tr
# 1 8512834  0.04314 - val
# 1 16401586 0.04222 - all

Store(data.ftrl.05.pred)
# 
# # fn.write.submission(data.ftrl.05.pred, "data.ftrl.05.pred")
