...
def load_pd_df(file_name, del_old=True, bin_suffix='.bin.pkl'):
    ret_val = None
...
def load_model_bin(model_file):
    with gzip.open(model_file, 'rb') as model_file:
        return pkl.load(model_file)


def save_model_bin(model, model_file):
    with gzip.open(model_file, 'wb') as model_file:
        pkl.dump(model, model_file, pkl.HIGHEST_PROTOCOL)
